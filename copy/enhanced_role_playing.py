...
def run_society(
    ...
    round_limit: int = 15,
    log_callback = None , # 新增：日志回调函数
    task_complete_callback = None
    ...
    for _round in range(round_limit):
        ...
        _data = {
            ...
            "assistant": assistant_response.msg.content[:-len("Next request.")].strip()
        ...
        if log_callback:
            if '<CAMEL_TASK_DONE>' in _data['user']:
                task_complete_callback({
                    "answer": _data['assistant'],
                })
                # 提前结束
                assistant_response.terminated=True
            else:
                log_callback(_data)
    ...
    logger.info(f"run society complete")
